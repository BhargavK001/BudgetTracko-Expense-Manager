...
function AccountCard({
    ...
    const balanceColor = useMemo(() => ({ 
        color: masked ? tokens.textMuted : isNegative ? '#F43F5E' : tokens.textPrimary 
    }), [masked, isNegative, tokens]);
    ...
    return (
        <AnimatedPressable
            style={[styles.card, { backgroundColor: tokens.cardSurface, borderColor: tokens.borderSubtle }]}
            onPress={onPress}
        ...
        >
            ...
            <View style={styles.info}>
                <Text style={[styles.name, { color: tokens.textPrimary }]} numberOfLines={1}>{name}</Text>
                ...
                {hasLimit && !masked && (
                    <View style={styles.creditWrap}>
                        <View style={[styles.creditBarBg, { backgroundColor: isDarkMode ? 'rgba(255,255,255,0.05)' : '#F2F2F7' }]}>
                            <View style={[styles.creditBarFill, creditFillStyle]} />
                        </View>
                        <View style={styles.creditLabels}>
                            <Text style={[styles.creditTxt, { color: tokens.textMuted }]}>
            ...
            <View style={styles.right}>
                <Text style={[styles.balLabel, { color: tokens.textMuted }]}>Balance</Text>
...
const styles = StyleSheet.create({
    ...
    creditBarBg: { height: 4, borderRadius: 2, overflow: 'hidden' },
    ...
    balLabel: { fontSize: 9, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.3, marginBottom: 2 },
